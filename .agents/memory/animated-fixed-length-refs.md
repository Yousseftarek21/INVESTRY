...
**How to apply:** For animated values tied to a dynamically-sized/filtered list, key the animated values by a stable ID (label) in a `Record<string, Animated.Value>` and lazily create entries on demand. Avoid approach (b) "pre-allocate a fixed max-size array" — it recurred as the same crash in `AllocationBar.tsx`'s `OverviewBar` when a 5th segment (Personal Assets) was added but `MAX_SEGMENTS` stayed hardcoded at 4; that component now also uses the keyed-by-label approach. Treat any hardcoded max-segment-count constant as a landmine — prefer the keyed pattern everywhere over fixed-size arrays. Also check any `ErrorBoundary`/fallback UI doesn't itself depend on context providers declared *below* the boundary in the tree — otherwise a real crash cascades into a second crash when the fallback renders.
