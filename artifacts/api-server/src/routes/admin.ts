...
// sourced from the phone's OS-level language, not INVESTRY's own in-app
// toggle) decides which version they get; 'ar' users get the Arabic pair,
// everyone else gets the plain title/body. `route` is likewise optional —
// an in-app path (see useNotificationTapRouting.ts's DESTINATION map) that
// tapping the notification navigates straight to.
