...
// PUT /api/account/language — syncs the phone's OS-level language (NOT
// INVESTRY's own in-app language toggle) so every server-sent push knows
// which language to send in. Upserts rather than requiring the row to
// already exist, same pattern as push.ts's /push/register.
