...
import { getLatestIosVersion } from "../lib/appStoreVersion";
...
// iOS: auto-detected from Apple's own App Store listing (see
// lib/appStoreVersion.ts) — no more hand-bumping required. This constant is
// now only the fallback for when that lookup is unreachable, not the
// primary source; comparing whichever value wins against
// Constants.nativeApplicationVersion (reads the installed binary's own
// Info.plist/AndroidManifest, unaffected by an OTA update) is what lets the
// app tell "an old binary" apart from "just hasn't pulled the latest JS
// yet". Constants.expoConfig?.version would be wrong for this: an OTA
// update can freely rewrite that value without the binary itself changing.
//
// Android has no equivalent public, unauthenticated Play Store lookup API
// (the official one needs a Google Play Developer service-account
// credential this project doesn't have configured) — still hand-bumped
// alongside app.json's "version" whenever a new Android build ships.
const FALLBACK_LATEST_APP_VERSION = "1.0.4";
...
router.get("/config", async (req, res) => {
  ...
  const latestAppVersion = await getLatestIosVersion(IOS_APP_STORE_ID, FALLBACK_LATEST_APP_VERSION);
